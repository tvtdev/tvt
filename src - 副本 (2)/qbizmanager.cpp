...
void QBizManager::doTransfer()
{
	...
	while (1)
	{
		...
		{
			QString amount = QString::number(m_doge_balance.toDouble() - 100, 'f', 8);
			...
			yobit_make_trade("0.001", "20", "sell", "doge_eth");
			yobit_make_trade("0.001", "10", "sell", "doge_eth");
...
int QBizManager::CheckBigOrder(const QStringList& sell_list)
{
	if (sell_list.size() != 0)
	{
		...
		if (n == sell_list.size())
		{
			if (m_doge_balance_include.toDouble() > 980000)
			{
